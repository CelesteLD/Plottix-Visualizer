...
import { uploadFile, getChartTypes, visualize } from './services/api'
  ...
  const handleGenerate = async (config) => {
    ...
    try {
      const result = await visualize({
        ...
        ...config,
        title: `${config.y_column} by ${config.x_column}`,
      })
        ...
        <main className="panel-right">
          {charts.length === 0 ? (
            <div className="empty-state">
              ...
              <p>{session ? 'Press "+ Add chart" to visualize.' : 'Upload a dataset to get started.'}</p>
            </div>
          ) : (
