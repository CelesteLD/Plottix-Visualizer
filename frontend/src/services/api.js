...
export async function visualizeCorrelogram({ session_id, columns, title }) {
  const { data } = await api.post('/visualize-correlogram', {
    session_id,
    columns,
    title: title || 'Correlogram',
  })
  return data // { chart_type: "correlogram", title, data: [{x, y, value}], columns }
}